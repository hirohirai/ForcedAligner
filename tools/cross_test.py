...
def save_data(X,Y,y):
    ...
    for ii in range(5):
        fn = f'data_t/in_test/s1/A{ii:02}'
        np.savez_compressed(fn, mgc=None, spc=None, mfcc=X[ii * 300:(ii + 1) * 300])
        fn = f'data_t/tgt_test/s1/A{ii:02}'
        np.savez_compressed(fn, target=y_[ii * 300:(ii + 1) * 300])
...
def main(args):
    ...
    # 損失関数の設定(クロスエントロピー誤差)
    criterion = torch.nn.CrossEntropyLoss()  # この中でソフトマックス関数と同じ処理をしている
    ...
    for epoch in range(loopn):
        for ix in range(120):
            fn = f'./data_t/in_test/s1/A{ix:03}.npz'
            in1 = np.load(fn)['mfcc']
            batch = torch.FloatTensor(in1[:,7,:])
            fn = f'./data_t/tgt_test/s1/A{ix:03}.npz'
            tgt = np.load(fn)['target']
            label = torch.LongTensor(tgt)
    ...
    with torch.no_grad():# 試験用データでは勾配を計算しない
        ...
        test_Xs = []
        test_Ys = []
        for nn in range(120, 150):
            fn = f'./data_t/in_test/s1/A{nn:03}.npz'
            in1 = np.load(fn)['mfcc']
            test_X = torch.FloatTensor(in1[:,7,:])
            for x in test_X:
                test_Xs.append(x[0])
                test_Ys.append(x[1])
                pred = net(x) #モデルの出力
    ...
    test_Xs = np.array(test_Xs)  # numpy arrayに変換
    test_Ys = np.array(test_Ys)  # numpy arrayに変換

    # データを描画
    plt.scatter(test_Xs, test_Ys, c=pred_labels, s=40, cmap=plt.cm.Spectral)
    plt.show()
