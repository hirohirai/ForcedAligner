...
def add_phr(ifn, ofn, mae_t, usiro_t):
    tg = TextGrid(ifn)
    ...
    for ix,ka in enumerate(kana.intervals):
        if ka.text[0] == '｜':
            ...
            if mae == mae_t and usiro == usiro_t:
                kana.intervals[ix].text = '，' + ka.text[1:]
...
def main(args):
    ...
    for ll in args.input:
        ...
        usiro = ee[3]
        if args.UL[0] == 'U':
            itgfn = f'{args.indir}/{abc.upper()}{num}.TextGrid'
        else:
            itgfn = f'{args.indir}/{abc.lower()}{num}.TextGrid'
        if args.UL[1] == 'U':
            otgfn = f'{args.outdir}/{abc.upper()}{num}.TextGrid'
        else:
            otgfn = f'{args.outdir}/{abc.lower()}{num}.TextGrid'
        if fnb in save_list:
            itgfn = otgfn

        add_phr(itgfn, otgfn, mae, usiro)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--indir', default='/home/hirai/work_local/Speech/DBS_/rt-atr503/TextGrid_sub_syuusei')
    parser.add_argument('--UL', default='UU')
    parser.add_argument('--outdir', '-o', default='tmpdir')
